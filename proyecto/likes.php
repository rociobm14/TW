            ...
            <?php }    
                ...
                if ($resultado) {
                    ...
                    if ($count > 0) {
                        ...
                        VALUES ('$idUsuario', '$idIncidencia', 'Like')");?>
                        <span class = "confirmacion-datos">Se ha añadido el like a la incidencia.</span><?php
                        $fecha = date('Y-m-d H:i:s');
                        $accion = "El usuario {$_SESSION['usuario']['email']} ha dado like a una incidencia.";
                        $insercionLog = mysqli_query($db, "INSERT INTO Logs (fecha, accion) VALUES ('$fecha', '$accion')");

                        $query = "SELECT positivas FROM Incidencias WHERE id = $idIncidencia";
                        $resultado = mysqli_query($db, $query);

                        if ($resultado) {
                            $fila = mysqli_fetch_assoc($resultado);
                            $positivas = $fila['positivas'];
                        } else {
                            echo "Error al ejecutar la consulta: " . mysqli_error($db);
}

                        $query = mysqli_query($db, "UPDATE Incidencias SET positivas = '" . mysqli_real_escape_string($db, $positivas+1) . "' WHERE id = '" . mysqli_real_escape_string($db, $idIncidencia) . "'");
